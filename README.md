...
- **反超提醒**：当有队伍总分超过 40 分后，实时追踪排名变化并屏幕提醒（新领跑者/被超越/反超鼓励，**反超并获胜**时全服特别庆祝）；可在游戏内用 **K 键** 或 `/race alerts` 随时开启/关闭（默认开启）
...
2. **了解规则**：进大厅后消息栏会提示**快捷键**（H 规则书 / P 积分 / U 进度 / Y 语言 / K 反超提醒开关）与常用指令（`/race start` 开始 / `/race settings` 设置 / `/race status` 状态）；管理员可在背包中用**红石粉设置修改器**随时调整比赛设置。
...
| `/race alerts [on\|off]` | 切换反超提醒（无参=切换；默认开启） |
| `/race point` | 打开积分映射书 |
...
2. 将 `structure_race-2.21.0.jar` 放入 **mods** 文件夹（客户端与服务端都放，单机只需客户端）；
3. 启动游戏，进入任意存档即可开始。
...
src/client/java/.../StructureRaceClient.java   # 客户端快捷键（H/P/U/Y/K）
