...
- **大厅保护**：队伍选择器/规则书/语言选择器**不可丢弃**（丢出即清除并自动补发），进大厅消息栏提示「按 Tab 查看组队情况」
- **比赛设置站**：大厅平台上的**命令方块 + 按钮 + 告示牌**，一键切换获胜模式 / 调整分数与时长 / 开始比赛，新玩家无需记指令
...
2. **了解规则**：进大厅后消息栏会提示常用指令（组队/积分/进度/召回）；平台北侧有**比赛设置站**（按钮 + 告示牌说明），管理员/房主可一键切换模式或调整分数与时长。
3. **组队**：右键背包中的**队伍选择器（指南针）**打开选队界面，或输入 `/race join <颜色>`；右键**语言选择器（下界之星）**可随时切换语言。
4. **开始**：管理员输入 `/race start`（或点击设置站「开始比赛」按钮），全体 **5 秒倒计时**后传送至主世界出生点，背包被清空，时间重置为白天。
...
| `/race settings` | 打开**比赛设置 GUI**（切换模式 / 调整获胜分数与限时时长，无需记指令） |
...
2. 将 `structure_race-2.10.0.jar` 放入 **mods** 文件夹（客户端与服务端都放，单机只需客户端）；
3. 启动游戏，进入任意存档即可开始。
